...
class AuthorizationStateOperationalTest {
  ...
  void cleanupIsBoundedAndCannotDeleteActiveOrControlKeys() {
    ...
    for (int i = 0; i < 120; i++) bytes.set("auth:g1:authorization:" + i, new byte[] {1});
  ...
  void healthMetricsAndLogsContainOnlyBoundedOperationalDimensions() {
    AuthorizationStateProperties properties = AuthorizationStateRoutingTest.properties(
        AuthorizationStateProperties.Role.HOME, 1);
    SimpleMeterRegistry meters = new SimpleMeterRegistry();
    AuthorizationStateTelemetry telemetry = new AuthorizationStateTelemetry(meters, properties);
    InMemoryRedisByteStore bytes = new InMemoryRedisByteStore();
    bytes.available(false);
    AuthorizationStateManager state = new AuthorizationStateManager(properties,
        new RedisControlRepository(bytes, "auth"), new RedisGenerationCleanup(bytes, "auth", 100), telemetry,
        new AuthorizationStateRoutingTest.MutableClock(), duration -> {}, Runnable::run);
    ...
    assertThat(meters.getMeters()).allSatisfy(meter -> assertThat(meter.getId().getTags())
        .allSatisfy(tag -> assertThat(tag.getKey()).isIn("role", "mode", "from", "to", "outcome")));
    String diagnostics = health + meters.getMeters().toString()
        + logs.list.stream().map(ILoggingEvent::getFormattedMessage).toList();
    assertThat(diagnostics).doesNotContain("redacted", "user:admin", "session-id", "authorization-code",
  ...
  void filterKeepsMetadataAvailableAndGatesStatefulEndpoints() throws Exception {
    InMemoryRedisByteStore bytes = new InMemoryRedisByteStore();
    bytes.available(false);
    AuthorizationStateManager cloud = AuthorizationStateRoutingTest.manager(bytes,
  ...
  void validatesEnabledRedisAndRecoveryConfiguration() {
    try (var factory = Validation.buildDefaultValidatorFactory()) {
      ...
      AuthorizationStateProperties valid = AuthorizationStateRoutingTest.properties(
          AuthorizationStateProperties.Role.CLOUD, 3);
      AuthorizationStateProperties invalid = new AuthorizationStateProperties(true,
          AuthorizationStateProperties.Role.CLOUD,
          new AuthorizationStateProperties.Redis("", 6379, "", "", "bad:namespace"), 3,
          Duration.ZERO, Duration.ofSeconds(15), Duration.ofSeconds(30), Duration.ofSeconds(30),
          Duration.ofSeconds(2), Duration.ofSeconds(3), 100, Duration.ofMinutes(5));
