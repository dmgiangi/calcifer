...
class ForwardAuthControllerTest {
  private static final IdentityProperties PROPERTIES = new IdentityProperties("https://auth.calcifer.tech", "dem.gianluigi@gmail.com", "user:admin", "file:key",
  ...
  void acceptsListFormOfScopeClaim() {
    JwtDecoder decoder = Mockito.mock(JwtDecoder.class);
    Jwt jwt = new Jwt("token", Instant.now(), Instant.now().plusSeconds(60), Map.of("alg", "RS256"),
        Map.of("iss", "https://auth.calcifer.tech", "aud", List.of("grafana"), "scope", List.of("grafana.api"), "sub", "grafana-api"));
    when(decoder.decode("token")).thenReturn(jwt);
    var response = new ForwardAuthController(decoder, PROPERTIES).authenticate("Bearer token");
    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);
  }
  ...
  void rejectsLookalikeScope() {
    JwtDecoder decoder = Mockito.mock(JwtDecoder.class);
    Jwt jwt = new Jwt("token", Instant.now(), Instant.now().plusSeconds(60), Map.of("alg", "RS256"),
        Map.of("iss", "https://auth.calcifer.tech", "aud", List.of("grafana"), "scope", "not-grafana.api", "sub", "grafana-api"));
    when(decoder.decode("token")).thenReturn(jwt);
    var response = new ForwardAuthController(decoder, PROPERTIES).authenticate("Bearer token");
  ...
  void rejectsWrongIssuer() {
    JwtDecoder decoder = Mockito.mock(JwtDecoder.class);
    Jwt jwt = new Jwt("token", Instant.now(), Instant.now().plusSeconds(60), Map.of("alg", "RS256"),
        Map.of("iss", "https://other.example", "aud", List.of("grafana"), "scope", "grafana.api", "sub", "grafana-api"));
    when(decoder.decode("token")).thenReturn(jwt);
    var response = new ForwardAuthController(decoder, PROPERTIES).authenticate("Bearer token");
    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.FORBIDDEN);
  }
  ...
  void rejectsWrongAudience() {
    JwtDecoder decoder = Mockito.mock(JwtDecoder.class);
    Jwt jwt = new Jwt("token", Instant.now(), Instant.now().plusSeconds(60), Map.of("alg", "RS256"),
        Map.of("iss", "https://auth.calcifer.tech", "aud", List.of("other"), "scope", "grafana.api", "sub", "grafana-api"));
    when(decoder.decode("token")).thenReturn(jwt);
    var response = new ForwardAuthController(decoder, PROPERTIES).authenticate("Bearer token");
    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.FORBIDDEN);
  }
  ...
  void rejectsExpiredMachineToken() {
    JwtDecoder decoder = Mockito.mock(JwtDecoder.class);
    Jwt jwt = new Jwt("token", Instant.now().minusSeconds(120), Instant.now().minusSeconds(60), Map.of("alg", "RS256"),
        Map.of("iss", "https://auth.calcifer.tech", "aud", List.of("grafana"), "scope", "grafana.api", "sub", "grafana-api"));
    when(decoder.decode("token")).thenReturn(jwt);
    var response = new ForwardAuthController(decoder, PROPERTIES).authenticate("Bearer token");
