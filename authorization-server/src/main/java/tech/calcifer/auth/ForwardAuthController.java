  ...
  ResponseEntity<Void> authenticate(@RequestHeader(value = HttpHeaders.AUTHORIZATION, required = false) String authorization) {
    ...
    try {
      ...
      if (scopeClaim instanceof String scope) {
        ...
        scopes = scopeValues.stream().filter(String.class::isInstance).map(String.class::cast)
            .collect(java.util.stream.Collectors.toUnmodifiableSet());
      } else {
        scopes = Set.of();
      }
      if (!properties.issuer().equals(token.getIssuer().toString()) || audience == null
          || !audience.contains(properties.grafanaApi().audience())
          || !scopes.contains("grafana.api") || !properties.grafanaApi().id().equals(token.getSubject())) {
