...
class RedisControlRepository {
    ...
    ControlState read() {
        ...
        if (parsed < 1) {
            throw new StateUnavailableException("Redis generation is invalid");
        }
    ...
    boolean acquireGate(String owner, Duration ttl) {
        return store.eval(
            ACQUIRE_GATE,
            List.of(key("lease"), key("gate")),
            List.of(RedisByteStore.bytes(owner), RedisByteStore.bytes(Long.toString(ttl.toMillis())))
        ) == 1;
    }
