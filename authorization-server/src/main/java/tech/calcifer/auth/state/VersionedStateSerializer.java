...
final class VersionedStateSerializer<T> {
  ...
  byte[] serialize(T value) {
    ...
    return ByteBuffer.allocate(MAGIC.length + 1 + payload.length)
        .put(MAGIC).put(VERSION).put(payload).array();
  }

  T deserialize(byte[] encoded) {
    if (encoded == null || encoded.length <= MAGIC.length
        || !Arrays.equals(MAGIC, Arrays.copyOf(encoded, MAGIC.length))
