...
final class RoutingSessionRepository implements SessionRepository<MapSession> {
        ...
        try {
            if (route.owner() == StateRoute.Owner.REDIS) {
                if (!session.getOriginalId().equals(session.getId())) {
                    redis.unlink(java.util.List.of(key(route, session.getOriginalId())));
                }
                redis.set(
                    key(route, session.getId()),
                    serializer.serialize(new MapSession(session)),
            ...
            log.warn(
                "authorization_session_operation_failed operation=save owner={} reason={} missing_type={}",
                route.owner(),
                exception.getClass().getSimpleName(),
                serializationFailureType(exception)
            );
    ...
    public MapSession findById(String id) {
        ...
        if (route.owner() == StateRoute.Owner.UNAVAILABLE || !id.startsWith(route.identifierPrefix())) {
            return null;
        }
        try {
            ...
            if (session != null) {
                session.setSessionIdGenerator(() -> route.identifierPrefix() + UUID.randomUUID());
            }
            if (route.owner() == StateRoute.Owner.REDIS) {
                state.connectedOperationSucceeded();
            }
    ...
    public void deleteById(String id) {
        ...
        try {
            if (route.owner() == StateRoute.Owner.REDIS) {
                ...
                state.localSessions(route).remove(id);
            }
        } catch (RuntimeException exception) {
            if (route.owner() == StateRoute.Owner.REDIS) {
                state.connectedOperationFailed();
    ...
    private String serializationFailureType(Throwable failure) {
        for (Throwable current = failure; current != null; current = current.getCause()) {
            ...
            if (current instanceof NotSerializableException && message.matches("[A-Za-z0-9_.$\\[;]+")) {
                return message;
            }
        }
        return "unknown";
