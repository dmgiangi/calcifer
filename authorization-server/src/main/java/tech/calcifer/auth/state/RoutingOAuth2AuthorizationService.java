...
final class RoutingOAuth2AuthorizationService implements OAuth2AuthorizationService {
  ...
  public OAuth2Authorization findById(String id) {
    return execute("find-by-id", route -> route.owner() == StateRoute.Owner.REDIS
        ? redis.findById(route.generation(), id) : state.localAuthorizations(route).findById(id));
  }

  @Override
  public OAuth2Authorization findByToken(String token, OAuth2TokenType tokenType) {
    return execute("find-by-token", route -> route.owner() == StateRoute.Owner.REDIS
  ...
  private <T> T execute(String operationName, RouteOperation<T> operation) {
    ...
    Observation observation = Observation.createNotStarted("authorization.state.repository", observations)
    ...
    try (Observation.Scope ignored = observation.openScope()) {
      if (route.owner() == StateRoute.Owner.UNAVAILABLE) {
        StateUnavailableException failure =
            new StateUnavailableException("Authorization state is temporarily unavailable");
