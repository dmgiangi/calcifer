...
public record AuthorizationStateProperties(
    ...
    public boolean isEnabledRedisConfigurationValid() {
        return !enabled || (redis != null
            && hasText(redis.host())
            && hasText(redis.username())
            && hasText(redis.password()));
    }
    ...
    public boolean isRecoveryTimingValid() {
        return positive(probeInterval)
            && positive(minimumIsolation)
            && positive(stableSuccess)
            && positive(leaseTtl)
            && positive(gateTtl)
            && drain != null
            && !drain.isNegative()
            && gateTtl.compareTo(drain) > 0
            && leaseTtl.compareTo(drain) > 0
