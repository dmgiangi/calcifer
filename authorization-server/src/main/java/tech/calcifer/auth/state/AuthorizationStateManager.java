...
final class AuthorizationStateManager {
    ...
    synchronized StateRoute routeForStatefulRequest() {
        if (mode == Mode.ISOLATED) {
            return StateRoute.local(isolation.epoch());
        }
        try {
    ...
    synchronized void connectedOperationFailed() {
        ...
        if (properties.role() == AuthorizationStateProperties.Role.HOME
            && consecutiveFailures >= properties.failureThreshold()) {
            enterIsolation();
        }
    }
    ...
    private void probeForRecovery() {
        ...
        try {
            ...
            synchronized (this) {
                ...
                ready = !before(now, isolatedAt.plus(properties.minimumIsolation())) && !before(
                    now,
                    stableSince.plus(properties.stableSuccess())
                );
            }
        } catch (RuntimeException exception) {
            synchronized (this) {
    ...
    private void recover(RedisControlRepository.ControlState observed) {
        ...
        if (!control.acquireLease(owner, properties.leaseTtl()) || !control.acquireGate(owner, properties.gateTtl())) {
            return;
        }
        transition(Mode.RECOVERING);
        ...
        try {
            ...
            try {
                if (control.committed(owner, expected)) {
                    advanced = expected + 1;
                }
    ...
    private synchronized void finishRecovery(String owner, long newGeneration, Instant started) {
        ...
        log.info(
            "authorization_state_recovery_completed role={} generation={} duration_ms={}",
            properties.role(),
            newGeneration,
            elapsed.toMillis()
        );
    ...
    private IsolationState local(StateRoute route) {
        ...
        if (route.owner() != StateRoute.Owner.LOCAL || current == null || !current
            .epoch()
            .equals(route.isolationEpoch())) {
    ...
    AuthorizationStateProperties.Role role() {
        return properties.role();
    }

    String isolationEpoch() {
        return isolation == null ? null : isolation.epoch();
    }
    ...
    private void transition(Mode target) {
        ...
        if (previous != target) {
            log.info(
                "authorization_state_transition role={} from={} to={} generation={}",
                properties.role(),
                previous,
                target,
                generation
