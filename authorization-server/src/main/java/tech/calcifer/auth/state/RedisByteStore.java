...
class RedisByteStore {
    ...
    void set(String key, byte[] value, Duration ttl) {
        try (RedisConnection connection = connections.getConnection()) {
            connection
                .stringCommands()
                .set(bytes(key), value, Expiration.milliseconds(ttl.toMillis()), SetOption.UPSERT);
        }
    }

    boolean setIfAbsent(String key, byte[] value, Duration ttl) {
        try (RedisConnection connection = connections.getConnection()) {
            return Boolean.TRUE.equals(connection
                .stringCommands()
                .set(bytes(key), value, Expiration.milliseconds(ttl.toMillis()), SetOption.SET_IF_ABSENT));
        }
    }
    ...
    long eval(String script, List<String> keys, List<byte[]> arguments) {
        ...
        for (int i = 0; i < keys.size(); i++) {
            values[i] = bytes(keys.get(i));
        }
        for (int i = 0; i < arguments.size(); i++) {
            values[keys.size() + i] = arguments.get(i);
        }
        try (RedisConnection connection = connections.getConnection()) {
