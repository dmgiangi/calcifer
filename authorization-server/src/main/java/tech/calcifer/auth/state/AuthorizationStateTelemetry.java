...
final class AuthorizationStateTelemetry {
  ...
  AuthorizationStateTelemetry(MeterRegistry meters, AuthorizationStateProperties properties) {
    ...
    Gauge.builder("authorization.state.redis.reachable", redisReachable, value -> value.get() ? 1 : 0)
        .tag("role", role).register(meters);
    Gauge.builder("authorization.state.generation", generation, AtomicLong::get)
        .tag("role", role).register(meters);
    for (AuthorizationStateManager.Mode candidate : AuthorizationStateManager.Mode.values()) {
      Gauge.builder("authorization.state.mode", this, value -> value.mode == candidate ? 1 : 0)
          .tag("role", role).tag("mode", candidate.name()).register(meters);
    }
  }
  ...
  void transition(AuthorizationStateManager.Mode from, AuthorizationStateManager.Mode to) {
    mode = to;
    if (from != to) {
      meters.counter("authorization.state.transitions", "role", role,
  ...
  void recovery(String outcome, Duration duration) {
    ...
    Timer.builder("authorization.state.recovery.duration").tag("role", role).tag("outcome", outcome)
