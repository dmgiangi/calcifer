//
// DigitalOutputHandler.h - Handler for digital output pins
//
...
/**
 * @brief Handler for OUTPUT_DIGITAL mode.
 * Subscribes to MQTT command topic and publishes state to state topic.
 */
class DigitalOutputHandler : public IDeviceHandler {
public:
    PinModeType getHandledMode() const override { return OUTPUT_DIGITAL; }
