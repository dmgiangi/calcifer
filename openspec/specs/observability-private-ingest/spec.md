...
metrics to Thanos Receive and logs to Loki.
...
- **WHEN** Alloy Home sends a valid remote-write request over WireGuard with the
  configured TLS and BasicAuth credentials
...
The system SHALL provide an HTTPS endpoint on `calcifer-cloud` at the private
WireGuard-only Traefik NodePort `32443` that accepts Loki push requests from
the authenticated Home Alloy and routes them to Loki.
...
- **THEN** the endpoint SHALL forward the request to Loki and return the backend
  result
