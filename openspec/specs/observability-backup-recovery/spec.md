...
The system SHALL configure scheduled Velero File System Backups powered by Kopia to capture the persistent volumes of VictoriaMetrics and VictoriaTraces while excluding VictoriaLogs.

#### Scenario: Scheduled backup execution
- **WHEN** the scheduled Velero backup triggers
- **THEN** Velero node-agent SHALL back up the metrics and traces PVC contents to the designated Azure Blob backup storage location using deduplication and encryption.
...
The system SHALL support restoring VictoriaMetrics and VictoriaTraces PVC data from Azure Blob Storage into newly provisioned local PVCs.
