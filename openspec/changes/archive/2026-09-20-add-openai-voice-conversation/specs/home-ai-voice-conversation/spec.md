...
Fornisce una modalità vocale esplicita per conversare in italiano con l'agente Google Gemini ufficiale di Home Assistant, mantenendo il contesto tra i turni e separandola dai normali comandi domotici locali.
...
- **WHEN** the user says `smetti di pensare`, `basta`, `fine conversazione`, or
  `torna in ascolto`
- **THEN** the Google Gemini agent SHALL provide a brief confirmation without
