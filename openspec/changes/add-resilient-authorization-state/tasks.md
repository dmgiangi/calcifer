...
- [x] 10.1 Verify that `kubectl` can access both `calcifer-cloud` and `calcifer-home`, that each context targets the intended K3s cluster and node, and that no check uses the wrong cluster.
- [x] 10.2 Verify Flux health on both contexts with `flux get sources all`, `flux get kustomizations`, and the relevant reconciliation status; confirm the expected Git revision is applied, no Kustomization is stalled, and no reconciliation error is present.
...
The checks marked above reflect the recorded successful runs: both Kubernetes contexts, Flux reconciliation, live resource and observability checks, Redis persistence and access controls, WireGuard/private reachability, negative exposure checks, automatic recovery from `CONNECTED` to `ISOLATED` and back to `CONNECTED` with exactly one generation advance, and user confirmation that the retired Google redirect URIs were removed.

Release `0.1.22` is deployed on both clusters. The post-fix outage run verified Home Google HTTP `503` in `ISOLATED`, local login HTTP `200`, Cloud continuity, a visible `RECOVERING` gate, generation `9` to `10`, and return to `CONNECTED` without a second advance. The user confirmed that the two retired Google redirect URIs were removed, leaving only the canonical callback. Tasks 7.3, 9.3–9.6, 10.6, 10.12, and 10.13 remain open only where they require interactive authenticated flows, persisted authenticated-session evidence, or live JWT/fencing validation.